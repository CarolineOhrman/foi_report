---
title: "Run 250130_M01834_0151_JKLM1"
...
current_run_id <- "250130_M01834_0151_000000000-JKLM1"
current_run <- NULL
...
**Influenza A surveillance and subtyping from respiratory samples**

**Created by:** Thomas Holm  
**Last modified by:** Caroline Öhrman on 2025-01-31
...
viral_data$Viral_Percentage <- viral_data$Viral_Reads /
  (viral_data$Viral_Reads + viral_data$Host_Reads) *
  100

# Viral abundance distribution
p1 <- ggplot(viral_data, aes(x = Viral_Percentage)) +
  geom_histogram(
    bins = max(8, min(12, sample_count / 3)),
    fill = "purple",
    alpha = 0.7,
    color = "white"
  ) +
  labs(
    title = "Viral Read Percentage Distribution",
    x = "Viral Reads (%)",
    y = "Frequency"
  ) +
  theme_minimal()
...
p2 <- ggplot(viral_data, aes(x = Ct_Value, y = log10(Viral_Reads + 1))) +
  ...
  labs(
    title = "Ct Values vs Viral Read Count",
    x = "Ct Value",
    y = "Log10(Viral Reads + 1)"
  ) +
  theme_minimal()
...
if (n_assembled > 0) {
  ...
  datatable(
    consensus_data,
    caption = paste(
      "Consensus genome assembly results (",
      n_assembled,
      " samples)"
    ),
  ...
  cat(
    "No samples had sufficient viral reads for consensus assembly (threshold: 1000 reads)\n"
  )
}
```
...
if (n_assembled > 0) {
  ...
  p4 <- ggplot(variant_long, aes(x = Sample, y = Count, fill = Type)) +
    geom_col(alpha = 0.8) +
    labs(
      title = "Variant Distribution by Sample",
      x = "Sample",
...
if (n_assembled >= 3) {
  ...
  cluster_assignments <- data.frame(
    ...
    Cluster = paste(
      "Cluster",
      sample(1:n_clusters, n_assembled, replace = TRUE)
    ),
  ...
  p5 <- ggplot(cluster_df, aes(x = Cluster, y = Count)) +
    ...
    labs(
      title = "Phylogenetic Cluster Distribution",
      x = "Cluster",
      y = "Number of Samples"
    ) +
    theme_minimal()
  ...
  p6 <- ggplot(
    cluster_assignments,
    aes(x = Cluster, y = Distance_to_Reference)
  ) +
    geom_boxplot(fill = "lightblue", alpha = 0.7) +
    geom_jitter(width = 0.2, alpha = 0.6) +
    labs(
      title = "Genetic Distance to Reference by Cluster",
      x = "Cluster",
...
- **Platform used:** MiSeq
- **Data yield:** 9101.5 MB
- **Quality metrics:** Q30=7,0G=75,9%
...
- **Raw data:** `/mnt/storage/sequencing-data/MiSeq_FOI_LAB/250130_M01834_0151_000000000-JKLM1/`
- **Consensus genomes:** [Genomes folder](../genomes/250130_M01834_0151_000000000-JKLM1/)
- **Variant calls:** [Variants folder](../variants/250130_M01834_0151_000000000-JKLM1/)
- **QC reports:** [MultiQC Report](../qc/250130_M01834_0151_000000000-JKLM1_multiqc.html)
